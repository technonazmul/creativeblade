...
if($_SESSION['SELL_action']=='edit') {
	 $item_details=item_details($_SESSION['SELL_auction_id']);
     
	
}

	$listing_type=($atype)? $atype : 3;	
	$buy_now_only_checked=($_POST['buy_now_only'])? $_POST['buy_now_only']: $item_details['bn_only'];
	
	if($listing_type==1 && $buy_now_only_checked==1) {
	  $listing_type=3;
	}
	
	
   $listing_type_name= listing_type_name($listing_type);
	
    $select_type='<select name="listing_type" id="ltype" onchange="listIn(this.value);">
									<option value="3" '.select(3,$listing_type).' >'.$MSG['931'] .'</option>
									<option value="1" '.select(1,$listing_type).'>'.$MSG['642'] .'</option>
									<option value="2" '.select(2,$listing_type).'>'.$MSG['641'] .'</option>
			 </select>';
...
if($listing_type==2) {
	...
	$buy_now_price=($_POST['buy_now_price'])? $_POST['buy_now_price'] : $item_details['buy_now'];
}
		...
		$template->assign_vars(array(
				...
				'LISTING_TYPE_NAME'=>$listing_type_name,
				'BUY_NOW_ONLY_CHECKED'=>$buy_now_only_checked,
				'BUY_NOW'=>$buy_now
