...
if (!defined('FILE_CACHE_PREFIX')) {
	 ...
	 */
	define('FILE_CACHE_PREFIX', 'getthumb');
}
...
if (!defined('FILE_CACHE_DIRECTORY')) {
	 ...
	 *
	 */
	define ('FILE_CACHE_DIRECTORY', './uploaded/cache');
 ...
 */ {
	class mthumb {
		...
		protected function webidLink() {
			...
			if (isset($_GET['fromfile'])) { $_GET['src'] = $_GET['fromfile']; }
			if (isset($_GET['w'])) { $_GET['h'] = 300; $_GET['w'] = 400; }
			$_GET['zc'] = '2'; // can be  1 , 2 or 3
			
			if ($current_page === 'item.php') {
			    $_GET['zc'] = '1';
			    $_GET['h'] = 300;
			    $_GET['w'] = 400;
			}
			
			if ($current_page === 'browse.php') {
			    $_GET['zc'] = '3';
			    $_GET['h'] = 120;
			    $_GET['w'] = 180;
			}
			/*
			// The ideal would be to add/change image size (h & w) and add zoom (zc) 
			// inside the template but some conditions we could use:
			$current_page = basename(parse_url($_SERVER['HTTP_REFERER'],PHP_URL_PATH));
			if ($current_page === 'item.php') {
			    $_GET['zc'] = '1';
			    $_GET['h'] = $_GET['w'];
			}
