...
while ($row = $db->fetch())
{
	...
	$w=$system->SETTINGS['thumb_show'];
	$w='200';
	$template->assign_block_vars('featured', array(
			'ENDS' => $ends_string,
			'ID' => $row['id'],
			'BID' => $system->print_money($high_bid),
			'IMAGE' => (!empty($row['pict_url'])) ? 'getthumb.php?&w=' . $w . '&amp;fromfile=' . UPLOAD_FOLDER . $row['id'] . '/' . $row['pict_url'] : 'images/email_alerts/default_item_img.jpg',
...
$query = "SELECT id, title, starts, pict_url from " . $DBPrefix . "auctions
